...
async function createLaborExpenseForPayment({ employee, payment, payableHours, gross }) {
  const normalizedAmount = round2(gross);
  if (normalizedAmount <= 0) return null;

  const employeeName = employee?.name || 'Employee';
  const periodStartLabel = formatDateLabel(payment?.periodStart);
  const periodEndLabel = formatDateLabel(payment?.periodEnd);
  const hoursLabel = round2(payableHours).toFixed(2);
  ...
  return Expense.create({
    ...
    category: 'labor',
    date: new Date(),
    notes: `Employee: ${employeeName} | Hours: ${hoursLabel} | Period: ${periodStartLabel} to ${periodEndLabel} | Payroll ID: ${payment?._id}`,
  });
}
...
export async function calculatePayroll(periodStart, periodEnd, payableHoursByEmployee = {}) {
  ...
  for (const row of preview) {
    ...
    try {
      await createLaborExpenseForPayment({
